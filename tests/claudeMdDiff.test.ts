...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'llmguide-claudemd-'));
  db = openDb(path.join(tmpDir, 'db.sqlite'));
  savedHome = process.env.LLMGUIDE_HOME;
  process.env.LLMGUIDE_HOME = path.join(tmpDir, 'home');
});

afterEach(() => {
  db.close();
  if (savedHome === undefined) delete process.env.LLMGUIDE_HOME;
  else process.env.LLMGUIDE_HOME = savedHome;
...
describe('writeClaudeMdSuggestions', () => {
  it('writes CLAUDE.md.suggested in an existing project dir and never touches CLAUDE.md', () => {
    ...
    expect(body).toContain('generated by LLMGuide');
  ...
  it('falls back to <llmguideHome>/suggestions for projects that do not exist', () => {
    ...
    expect(target.startsWith(path.join(process.env.LLMGUIDE_HOME!, 'suggestions'))).toBe(true);
