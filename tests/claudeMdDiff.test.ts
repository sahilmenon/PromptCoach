...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'promptcoach-claudemd-'));
  db = openDb(path.join(tmpDir, 'db.sqlite'));
  savedHome = process.env.PROMPTCOACH_HOME;
  process.env.PROMPTCOACH_HOME = path.join(tmpDir, 'home');
});

afterEach(() => {
  db.close();
  if (savedHome === undefined) delete process.env.PROMPTCOACH_HOME;
  else process.env.PROMPTCOACH_HOME = savedHome;
...
describe('writeClaudeMdSuggestions', () => {
  it('writes CLAUDE.md.suggested in an existing project dir and never touches CLAUDE.md', () => {
    ...
    expect(body).toContain('generated by PromptCoach');
  ...
  it('falls back to <promptcoachHome>/suggestions for projects that do not exist', () => {
    ...
    expect(target.startsWith(path.join(process.env.PROMPTCOACH_HOME!, 'suggestions'))).toBe(true);
